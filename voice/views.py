...
#新版TTS要改成tts_path = ""，model_dir = "D:/ai_models/crowds_of_voices/home/ubuntu/home/yourtts-pth-0515"
    ...
    def upload(self, request):  ####post
        if request.method == 'POST' and request.FILES.getlist('sound'):
            ...
            for sound_file in sounds:
                ...
                UserSound.objects.filter(user=user).update(time=tim)
                UserSound.objects.filter(id=user_sound.id).update(description=name)
